...
public class CoinChange {
     ...
     * where r = a-coins[i] for all i's. For every remainder,
     * go through exactly the same process as before until either the remainder is 0
     * or less than 0 (meaning not a valid solution). With this idea,
     * the only remaining detail is to store the minimum number of coins needed to sum up to r
     ...
     * "使用dp解 通项公式为 res[i] = min([res[i-j] for j in coins if i-j>=0]) + 1
     * 就是要从0到amount挨个算,当前的这个数量一定是之前的某一个数量然后加上了一个coin里的数量得来的,
     * 然后就是取最小的即可 注意一下初始值的设置  设置的是一个不可能得数从而才能知道什么时候返回-1"
     */
