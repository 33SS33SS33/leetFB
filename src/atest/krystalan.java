...
public class krystalan {
     ...
     *
     ...
     hashtable stores key-value pair in Map.Entry static nested class implementation. Any non-null object can be used as a key or value, hashtable works on hashing algorithm and uses hashCode() and equals() method in put and get methods.When we call put method by passing key-value pair, hashtable uses Key hashCode() with hashing to find out the index to store the key-value pair. The Entry is stored in the LinkedList, so if there are already existing entry, it uses equals() method to check if the passed key already exists, if yes it overwrites the value else it creates a new entry and store this key-value Entry. When we call get method by passing Key, again it uses the hashCode() to find the index in the array and then use equals() method to find the correct Entry and return it’s value. Below image will explain these detail clearly.
     优点缺点：time complexity of O (1). Dynamic variable length data storage
     超级快速的查询速度，如果有人问你什么数据结构可以达到O(1)的时间复杂度，没错是HashMap
     动态的可变长存储数据（和数组相比较而言）
     缺点：need extra computing a hash value.If not handled properly will take up extra space需要额外计算一次hash值,如果处理不当会占用额外的空间
     ...
     * and load factor.
     ...
     do recursion if there is no base case -> stack overflow;..

     一个用bst编辑的字典，上面有各种product name and category. 求找到一种具体产品的时间复杂度

     there is a search method called binary search method, it makes full use of the ordered relationship between elements, n elements can be divided into two roughly equal parts, compare the middle element with the one want to find, decide to do binary search in left part or right part,  in the worst case with O (log n)
